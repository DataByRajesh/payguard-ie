...
export default function ReconciliationError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <div className="flex flex-col items-start gap-3 rounded-md border border-rose-200 bg-rose-50 p-6">
      <h2 className="text-sm font-semibold text-rose-800">Unable to load reconciliation</h2>
      <p className="text-sm text-rose-700">
        An error occurred while retrieving reconciliation data. {error.digest ? `Reference: ${error.digest}` : null}
      </p>
      <button
        type="button"
        onClick={reset}
