...
## Authenticated, but not yet authorized (Cloud Phase 2.1)
...
**Consequence — still no authorization:** every Server Action verifies *who* is acting (a real, logged-in `User`) but not yet *whether that role is allowed to* — any logged-in user can assign, approve, or reject any case regardless of role. A production version would need per-action, role-based authorization on every mutation; this lands in Cloud Phase 2.2, tracked against the roles already named as a placeholder in `app/settings/page.tsx`. The "approver must differ from resolver" rule ([EXCEPTION_LIFECYCLE.md#approval-and-closure](EXCEPTION_LIFECYCLE.md#approval-and-closure)) is unaffected by this gap — it checks real user identity, not role, and still requires actually logging in as someone else to get around it (no more switching a dropdown).

**Cloud demo mode is unchanged:** the public Vercel demo still runs with `DEMO_READ_ONLY=true` ([CLOUD_DEPLOYMENT.md](CLOUD_DEPLOYMENT.md)), making every mutating Server Action a no-op regardless of who's logged in. This remains a blanket read-only switch, not authorization, and composes independently of login — the public demo is read-only for every user, not just unauthenticated visitors.
