...
Cloud Phase 2.1 added real login: `proxy.ts` (Next.js 16's renamed `middleware.ts` — see its file header) redirects any request without a valid session cookie to `/login`; `lib/actions/auth.ts` verifies a password (`lib/auth/password.ts`, `node:crypto.scrypt`) and issues a signed, stateless session cookie (`lib/auth/session.ts`, HMAC-SHA256, `SESSION_SECRET`). `lib/acting-user.ts` looks up the `User` row for the current session and is what every Server Action attributes its mutation to. Sessions are stateless — revocation only works via `User.isActive`, checked on every request — see [SECURITY_AND_LIMITATIONS.md](SECURITY_AND_LIMITATIONS.md) for that trade-off. Cloud Phase 2.2 added role-based authorization on top: `lib/auth/permissions.ts`'s `requirePermission(actor, permission)` gates every mutating Server Action by the actor's role, right after `getActingUser()` resolves it. Cloud Phase 2.3 made `AuditEvent.actorUserId` a required FK to `User` (previously a free-text name) — see [EXCEPTION_LIFECYCLE.md#audit-trail](EXCEPTION_LIFECYCLE.md#audit-trail). Cloud Phase 2.5 added `lib/rate-limit.ts`'s `checkRateLimit(userId, action)` as the next guard clause after `requirePermission()`, and `lib/logger.ts` for structured JSON logging in place of ad hoc `console.error` calls — see [SECURITY_AND_LIMITATIONS.md](SECURITY_AND_LIMITATIONS.md) for the fixed-window trade-off.
