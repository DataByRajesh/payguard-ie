...
Since the Cloud Phase 1A migration to Postgres-over-Docker, this class of flakiness has shown up somewhat more often than it did against SQLite's in-process, zero-latency writes — each Server Action now makes a real network round-trip (loopback, but still a TCP round-trip through Docker Desktop's VM on this machine) instead of a synchronous file read. Click/status-wait timeouts in `exception-lifecycle.spec.ts` and `uat.spec.ts` were widened accordingly (15s → 20s) rather than chasing this further; it remains an environment/timing characteristic of this sandbox, not a defect introduced by the Postgres migration — the same journeys run 100% reliably through the equivalent Vitest integration tests, which hit the real database directly with no browser or dev-server layer in between.
